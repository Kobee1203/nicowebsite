/*
Modification made by: Toni Van de Voorde (toni.vdv@mg-x.eu)
...
SplitPane.handleWidth = 5; // Width of the handle
...
SplitPane.prototype = {
    ...
    options:
        ...
        ~ New option patched by Toni Van de Voorde (toni.vdv@mg-x.eu) ~
    ...
    */
    ...
    /*
     	Patched by Toni Van de Voorde (toni.vdv@mg-x.eu)
     	function: mouseOver
     */
    ...
    update: function(event) {
        if (this.active) {
            ...
            this.divider.style.backgroundColor="#CFCFCF";
            
            /*
             * patched by Toni Van de Voorde (toni.vdv@mg-x.eu)
             * 
             * Mozilla Firefox bugfix : In firefox the scrollbars only appears
             * when the content height is higher then the firefox window. When this occurs
             * the container width is reduced, and the variable containerWidth must be updated.
             */
            ...
            // Limit width of div2
            if (new_div2_width < this.div2_min_width) {
                canResize = false;
            }
    ...
    /*
        ...
        function: setStartWidth
            sets all widths of the divider, container, div1 and div2
    */
    setStartWidth: function(event) {
      ...
      // Bug fix:  remove margins to the div1 width
      var marginLeft = parseInt(this.div1.getStyle('margin-left'));
      this.start_div1_width -= marginLeft || 0;
	    var marginRight = parseInt(this.div1.getStyle('margin-right'));
	    this.start_div1_width -= marginRight || 0;
      
      this.start_div2_left = this.getX(this.div2);
      
      this.start_div2_width = this.getWidth(this.div2);
      // Bug fix:  remove margins to the div2 width
      var marginLeft = parseInt(this.div2.getStyle('margin-left'));
      this.start_div2_width -= marginLeft || 0;
	    var marginRight = parseInt(this.div2.getStyle('margin-right'));
	    this.start_div2_width -= marginRight || 0;
...
SplitPane.updateAllDividers = function () {
  for(i=0; i<SplitPane.cache.length; i++) {
    SplitPane.cache[i].setDividerX();
